...
/*   Updated: 2024/07/26 10:31:51 by nbenyahy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_exec_ret	*and_node(t_command *command, char **ev, t_exec_ret *ret,
		t_env *env)
{
	t_exec_ret	*tmp;
...
bool	pipeline_node(t_command **command, char **ev, t_exec_ret *ret,
		t_env *env)
{
	int			fd[2];
	t_exec_ret	*tmp;
...
void	subshell_childp(t_command *command, t_exec_ret *ret, t_env_d *d_env,
		char c)
{
	int	kk;
...
t_exec_ret	*subshell_node(t_command *command, t_exec_ret *ret, t_env_d *d_env,
		char c)
{
	bool	found_in;
	int		f;
...
void	child_proc(t_command *command, char c, bool f, t_env_d *d_env)
{
	...
	{
		if ((command->outfiles && command->outfd == -1) || (command->infd
				&& f == true) || do_builtin(command, d_env->env) == -1)
			exit(1);
		exit(0);
	}
